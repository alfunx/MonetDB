...
stmt *
stmt_cpd(sql_allocator *sa, stmt *op1, list *l)
{
	stmt *s = stmt_create(sa, st_cpd);
	s->op1 = op1;
	s->op2 = stmt_list(sa, l);
	s->op4.lval = l;
	s->nrcols = 1;
...
stmt *
stmt_gathersqrt(sql_allocator *sa, stmt *op1, stmt *op2)
{
	stmt *s = stmt_create(sa, st_gathersqrt);
	s->op1 = op1;
	s->op2 = op2;
	s->nrcols = 1;
	return s;
