...
static sql_rel * rel_matrixtraquery(mvc *sql, sql_rel *rel, symbol *q);
...
query_exp_optname(mvc *sql, sql_rel *r, symbol *q)
{
	...
	case SQL_MATRIXTRA:
	{
		sql_rel *tq = rel_matrixtraquery(sql, r, q);
	...
	case SQL_MATRIXQQR:
	{
		sql_rel *tq = rel_matrixqqrquery(sql, r, q);
	...
	case SQL_MATRIXRQR:
	{
		sql_rel *tq = rel_matrixrqrquery(sql, r, q);
...
static list *
gen_exps_list(mvc *sql, sql_rel *rel, dlist *s)
{
	list *l = sa_list(sql->sa);
	for (dnode *n = s->h; n; n = n->next) {
...
rel_matrixaddquery(mvc *sql, sql_rel *rel, symbol *q)
{
	...
	if (list_length(rel->lexps) != list_length(rel->rexps)) {
		sql_error(sql, 02, "MATRIX ADD: number of selected columns from tables '%s' and '%s' don\'t match", rel_name(l_rel)?rel_name(l_rel):"", rel_name(r_rel)?rel_name(r_rel):"");
		rel_destroy(rel);
		return NULL;
	}
	...
	append_exps_except(exps, sql, r_rel, rel->rexps);
...
rel_matrixsqrtquery(mvc *sql, sql_rel *rel, symbol *q)
{
	...
	rel->lord = gen_orderby(sql, l_rel, tab2);
	rel->lexps = gen_exps_list(sql, l_rel, tab3);
	rel->rexps = gen_exps_list(sql, l_rel, tab4);
...
rel_matrixtraquery(mvc *sql, sql_rel *rel, symbol *q)
{
	...
	append(exps, exp_column(sql->sa, el->rname, BL_NAME, exp_subtype(el), 0, 0, 0));
	append(exps, exp_column(sql->sa, er->rname,     nme, exp_subtype(er), 0, 0, 0));
...
rel_matrixrqrquery_simple(mvc *sql, sql_rel *rel, symbol *q)
{
	...
	// rqr relation
	rel = rel_matrixrqr(sql->sa, l_rel, qqr_rel);
	rel->lord = gen_orderby(sql, l_rel, tab2);
	rel->lexps = gen_exps_list(sql, l_rel, tab3);
